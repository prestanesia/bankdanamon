...
class BankDanamon extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $config = Configuration::getMultiple(array('BANK_DANAMON_DETAILS', 'BANK_DANAMON_OWNER', 'BANK_DANAMON_ADDRESS'));
        ...
        $this->displayName = $this->l('Bank Danamon');
        $this->description = $this->l('Accept payments for your products via Bank Danamon transfer.');
    ...
    public function install()
    {
        ...
        $newState->module_name = 'bankdanamon';
        $newState->invoice = false;
        $newState->color = "#FCCE34";
        ...
        foreach ($languages as $language) {
            if ($language['iso_code'] == 'id') {
                $newState->name[(int)$language['id_lang']] = 'Menunggu pembayaran via Bank Danamon';
            } else {
                $newState->name[(int)$language['id_lang']] = 'Awaiting Bank Danamon Payment';
            }
        ...
        if ($newState->add()) {
            ...
            foreach ($languages as $language) {
                if ($language['iso_code'] == 'id') {
                    copy(dirname(__FILE__).'/mails/id/bankdanamon.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankdanamon.html');
                    copy(dirname(__FILE__).'/mails/id/bankdanamon.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankdanamon.txt');
                } else {
                    copy(dirname(__FILE__).'/mails/en/bankdanamon.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankdanamon.html');
                    copy(dirname(__FILE__).'/mails/en/bankdanamon.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankdanamon.txt');
                }
            }
        } else {
            return false;
    ...
    {
        if (!Configuration::deleteByName('BANK_DANAMON_DETAILS') || !Configuration::deleteByName('BANK_DANAMON_OWNER') || !Configuration::deleteByName('BANK_DANAMON_ADDRESS') || !parent::uninstall()) {
            return false;
        }
        return true;
    }
    ...
    public function hookDisplayPaymentEU($params)
    {
        ...
        $payment_options = array(
            'cta_text' => $this->l('Pay by Bank Danamon'),
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (in_array($state, array(Configuration::get('PS_OS_BANKDANAMON'), Configuration::get('PS_OS_OUTOFSTOCK'), Configuration::get('PS_OS_OUTOFSTOCK_UNPAID')))) {
            $this->smarty->assign(array(
    ...
    {
        $this->context->controller->addCSS(($this->_path).'bankdanamon.css', 'all');
    }
}
